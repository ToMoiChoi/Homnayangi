...
export default function Home(){
 ...
 {counterEnabled&&<p className="local-counter" title={language==='vi'?'Lượt mở trên trình duyệt này, lưu bằng cookie':'Spins on this browser, stored in cookies'}>{language==='vi'?'Cậu đã bốc thăm':'You have opened'} <strong>{localSpins===null?'—':new Intl.NumberFormat(language==='vi'?'vi-VN':'en-US').format(localSpins)}</strong> {language==='vi'?'lần rồi nè ₍ᐢ.ˬ.⑅ᐢ₎':'cases on this browser'}</p>}
 {result&&!spinning&&<p className="local-counter">{language==='vi'?'Món cậu vừa chọn được nè: ':'Last choice: '}<strong>{foodName(result,language)} ✨</strong></p>}
