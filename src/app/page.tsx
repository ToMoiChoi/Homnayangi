...
export default function Home(){
 ...
 const [budget,setBudget]=useState('50'),[custom,setCustom]=useState('50'),[veg,setVeg]=useState(false),[sound,setSound]=useState(true),[spinning,setSpinning]=useState(false),[result,setResult]=useState<Food|null>(null),[revealed,setRevealed]=useState(false),[copied,setCopied]=useState(false);
 ...
 {counterEnabled&&<p className="local-counter" title={language==='vi'?'Lượt quay trên trình duyệt này, lưu bằng cookie':'Spins on this browser, stored in cookies'}>{language==='vi'?'Bạn đã quay':'You have opened'} <strong>{localSpins===null?'—':new Intl.NumberFormat(language==='vi'?'vi-VN':'en-US').format(localSpins)}</strong> {language==='vi'?'lần trên thiết bị này':'times on this device'}</p>}
 {result&&!spinning&&<p className="local-counter">{language==='vi'?'Món vừa chọn: ':'Last choice: '}<strong>{foodName(result,language)}</strong></p>}
